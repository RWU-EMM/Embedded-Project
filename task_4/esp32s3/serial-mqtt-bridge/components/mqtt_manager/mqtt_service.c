 ...
 *  */
static uint8_t topic_matches(const char *filter, const char *topic)
{
    const char *f = filter;
    const char *t = topic;
...
static uint8_t transport_is_secure(mqtt_transport_t t)
{
                               ...
                               void *event_data)
{
    ...
    case MQTT_EVENT_PUBLISHED:
        ESP_LOGI(__func__, "published msg_id=%d", event->msg_id);
        break;
...
static esp_err_t create_client_locked(void)
{
    ...
    if (transport_is_secure(s_mqtt.cfg.transport))
    {
        ...
        c.broker.verification.skip_cert_common_name_check = 0;
    }
                               ...
                               int qos, int retain)
{
    ...
    if (id < 0)
    {
        ESP_LOGE(__func__, "publish failed topic=%s", topic);
        return ESP_FAIL;
    }
    ESP_LOGI(__func__, "publish queued topic=%s msg_id=%d", topic, id);
    return ESP_OK;
}
...
{
    if (!s_mqtt.initialized || !s_mqtt.lock)
        return 0;
    xSemaphoreTake(s_mqtt.lock, portMAX_DELAY);
    uint8_t c = (s_mqtt.state == MQTT_STATE_CONNECTED);
    xSemaphoreGive(s_mqtt.lock);
    return c;
}
