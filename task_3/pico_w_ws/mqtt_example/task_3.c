...
static void publish_temperature(MQTT_CLIENT_DATA_T *state)
{
    ...
    const char *temperature_key = full_topic(state, "/yTemp");
...
static void temperature_worker_fn(async_context_t *context, async_at_time_worker_t *worker)
{
    ...
    {
        INFO_printf("MQTT connection status: %d\n", state->connect_done);
    }
...
static void publish_heartbeat(MQTT_CLIENT_DATA_T *state)
{
    const char *topic = full_topic(state, "/heartbeat");

    char msg[32];
    uint32_t uptime = to_ms_since_boot(get_absolute_time()) / 1000;

    snprintf(msg, sizeof(msg), "alive:%lu", uptime);

    INFO_printf("Heartbeat -> %s : %s\n", topic, msg);
...
static void heartbeat_worker_fn(async_context_t *context, async_at_time_worker_t *worker)
{
    ...
    {
        INFO_printf("Waiting for MQTT...\n");
    }

    async_context_add_at_time_worker_in_ms(context, worker, 1000); // every 1 sec
}
...
static void mqtt_connection_cb(mqtt_client_t *client, void *arg, mqtt_connection_status_t status)
{
    ...
    if (status == MQTT_CONNECT_ACCEPTED)
    {
        ...
        heartbeat_worker.user_data = state;
...
int main(void)
{
    ...
    cyw43_arch_lwip_begin();
    INFO_printf("Connecting MQTT Server: %s\n", MQTT_SERVER);
