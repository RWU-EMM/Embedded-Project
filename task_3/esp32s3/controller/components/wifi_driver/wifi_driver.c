...
uint8_t g_got_wifi_data_flag = false;
uint8_t g_device_connected_flag = false;
 ...
 *
 * @return bool Returns true if successfully connected, otherwise false.
 */
...
void esp_start_wifi_driver(int wifi_op_mode, const char *ap_ssid, const char *ap_passwd, const char *sta_ssid, const char *sta_passwd)
{
    ...
    while ((wifi_connected != true) && (wifi_op_mode != 0))
    {
