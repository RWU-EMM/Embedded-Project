...
static esp_err_t copy_config(mqtt_cfg_owned_t *dst, const mqtt_service_config_t *src)
{
    ...
    ESP_RETURN_ON_FALSE(dst->host != NULL, ESP_ERR_NO_MEM, TAG, "strdup host failed");
                                    ...
                                    const char *data,  int data_len)
{
    ...
    if (!matched && !s_mqtt.default_handler) {
        ESP_LOGD(TAG, "no handler for topic=%s", topic_buf);
    }
}
    ...
    for (int i = 0; i < MQTT_MAX_SUBSCRIPTIONS; i++) {
        if (s_mqtt.subs[i].in_use) {
            int id = esp_mqtt_client_subscribe(s_mqtt.client,
                                               s_mqtt.subs[i].topic,
                                               s_mqtt.subs[i].qos);
            ESP_LOGI(TAG, "restore subscribe topic=%s qos=%d msg_id=%d",
...
static esp_err_t save_sub_locked(const char *topic, int qos)
{
    ...
    ESP_RETURN_ON_FALSE(idx >= 0, ESP_ERR_NO_MEM, TAG, "subscription table full");
...
{
    int idx = find_sub_idx_locked(topic);
    ESP_RETURN_ON_FALSE(idx >= 0, ESP_ERR_NOT_FOUND, TAG, "topic not subscribed");
    memset(&s_mqtt.subs[idx], 0, sizeof(s_mqtt.subs[idx]));
    return ESP_OK;
}
...
static esp_err_t build_uri_locked(void)
{
    ...
    ESP_RETURN_ON_FALSE(scheme, ESP_ERR_INVALID_ARG, TAG, "invalid transport");
    ESP_RETURN_ON_FALSE(s_mqtt.cfg.host, ESP_ERR_INVALID_ARG, TAG, "host is null");
    ...
    ESP_RETURN_ON_FALSE(n > 0 && (size_t)n < sizeof(s_mqtt.uri),
                        ESP_ERR_INVALID_SIZE, TAG, "URI too long");
    return ESP_OK;
}
...
static void log_transport_error(esp_mqtt_event_handle_t event)
{
    if (!event->error_handle) {
        ESP_LOGE(TAG, "MQTT_EVENT_ERROR: null error_handle");
        return;
    }
    switch (event->error_handle->error_type) {
        case MQTT_ERROR_TYPE_TCP_TRANSPORT:
            ESP_LOGE(TAG, "TCP/TLS error esp_tls=0x%x stack=0x%x sock=%d",
                               ...
                               void *event_data)
{
    ...
    switch ((esp_mqtt_event_id_t)event_id) {
        ...
        case MQTT_EVENT_CONNECTED:
            ...
            ESP_LOGI(TAG, "MQTT connected (uri=%s)", s_mqtt.uri);
...
static esp_err_t create_client_locked(void)
{
    ...
    ESP_RETURN_ON_ERROR(err, TAG, "URI build failed");
    ...
    if (transport_is_secure(s_mqtt.cfg.transport)) {
        ESP_RETURN_ON_FALSE(s_mqtt.cfg.ca_cert, ESP_ERR_INVALID_ARG, TAG,
    ...
    ESP_RETURN_ON_FALSE(s_mqtt.client, ESP_FAIL, TAG, "esp_mqtt_client_init failed");
...
esp_err_t mqtt_service_init(const mqtt_service_config_t *cfg)
{
    ...
    ESP_RETURN_ON_FALSE(cfg->host, ESP_ERR_INVALID_ARG,   TAG, "host is null");
    ESP_RETURN_ON_FALSE(!s_mqtt.initialized, ESP_ERR_INVALID_STATE, TAG, "already initialized");

    memset(&s_mqtt, 0, sizeof(s_mqtt));

    s_mqtt.lock = xSemaphoreCreateMutex();
    ESP_RETURN_ON_FALSE(s_mqtt.lock, ESP_ERR_NO_MEM, TAG, "mutex create failed");
...
{
    ESP_RETURN_ON_FALSE(s_mqtt.initialized,  ESP_ERR_INVALID_STATE, TAG, "not initialized");
...
esp_err_t mqtt_service_publish(const char *topic,
                               ...
                               int qos, int retain)
{
    ESP_RETURN_ON_FALSE(topic,   ESP_ERR_INVALID_ARG,   TAG, "topic is null");
    ...
    if (id < 0) { ESP_LOGE(TAG, "publish failed topic=%s", topic); return ESP_FAIL; }
    ESP_LOGD(TAG, "publish queued topic=%s msg_id=%d", topic, id);
    return ESP_OK;
}
...
esp_err_t mqtt_service_subscribe(const char *topic, int qos)
{
    ...
    ESP_RETURN_ON_FALSE(strlen(topic) < MQTT_MAX_TOPIC_LEN,
                        ESP_ERR_INVALID_ARG, TAG, "topic too long");
    ESP_RETURN_ON_FALSE(s_mqtt.initialized, ESP_ERR_INVALID_STATE, TAG, "not initialized");
    ...
    if (s_mqtt.client && s_mqtt.state == MQTT_STATE_CONNECTED) {
        ...
        ESP_LOGI(TAG, "subscribed topic=%s qos=%d msg_id=%d", topic, qos, id);
    } else {
        ESP_LOGW(TAG, "not connected — stored for reconnect: %s", topic);
    }
...
esp_err_t mqtt_service_unsubscribe(const char *topic)
{
    ...
    if (s_mqtt.client && s_mqtt.state == MQTT_STATE_CONNECTED) {
        ...
        ESP_LOGI(TAG, "unsubscribed topic=%s msg_id=%d", topic, id);
    }
...
{
    ESP_RETURN_ON_FALSE(filter, ESP_ERR_INVALID_ARG, TAG, "filter is null");
    ESP_RETURN_ON_FALSE(cb,     ESP_ERR_INVALID_ARG, TAG, "cb is null");
    ESP_RETURN_ON_FALSE(strlen(filter) < MQTT_MAX_TOPIC_LEN,
                        ESP_ERR_INVALID_ARG, TAG, "filter too long");
    ...
    for (int i = 0; i < MQTT_MAX_HANDLERS; i++) {
        if (!s_mqtt.handlers[i].in_use) {
            ...
            xSemaphoreGive(s_mqtt.lock);
            ESP_LOGI(TAG, "handler registered filter=%s", filter);
            return ESP_OK;
        }
    }

    xSemaphoreGive(s_mqtt.lock);
    ESP_LOGE(TAG, "handler table full (max=%d)", MQTT_MAX_HANDLERS);
...
esp_err_t mqtt_service_unregister_handler(const char *filter)
{
    ...
    for (int i = 0; i < MQTT_MAX_HANDLERS; i++) {
        if (s_mqtt.handlers[i].in_use &&
            ...
            xSemaphoreGive(s_mqtt.lock);
            ESP_LOGI(TAG, "handler unregistered filter=%s", filter);
            return ESP_OK;
        }
    }
