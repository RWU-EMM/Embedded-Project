...
static uint8_t s_batch_buf[sizeof(dsm_batch_hdr_t) +
                           UDP_BATCH_SIZE * sizeof(dsm_packet_t)] = {0};
...
/* Primary and Secondary UART Handles using your driver */
static uart_driver_handle_t s_uart_cmd;
static uart_driver_handle_t s_uart_ext;
...
static void adc_init(void) {
  ...
  ESP_LOGI(__func__, "ADC initialised (unit=1, ch=%d, atten=12dB)",
           ADC_CHANNEL);
}
...
static void ext_interface_init(void) {
  /* 1. Configure CMD_GPIO as Output (Default HIGH) */
  gpio_config_t io_conf_cmd = {
      .pin_bit_mask = (1ULL << EXT_CMD_GPIO),
      .mode = GPIO_MODE_OUTPUT,
      .pull_up_en = GPIO_PULLUP_ENABLE,
      .pull_down_en = GPIO_PULLDOWN_DISABLE,
      .intr_type = GPIO_INTR_DISABLE,
  };
  gpio_config(&io_conf_cmd);
  gpio_set_level(EXT_CMD_GPIO, 1); /* Default HIGH */

  /* 2. Configure RDY_GPIO as Input (Default HIGH via pull-up) */
  gpio_config_t io_conf_rdy = {
      .pin_bit_mask = (1ULL << EXT_RDY_GPIO),
      .mode = GPIO_MODE_INPUT,
      .pull_up_en = GPIO_PULLUP_ENABLE,
      .pull_down_en = GPIO_PULLDOWN_DISABLE,
      .intr_type = GPIO_INTR_DISABLE,
  };
  gpio_config(&io_conf_rdy);

  /* 3. Initialize Secondary UART using custom driver */
  memset(&s_uart_ext, 0, sizeof(s_uart_ext));
  s_uart_ext.config = (uart_driver_config_t){
      .uart_port = UART_NUM_1,
      .baudrate = _115200,
      .data_bits = UART_DATA_8_BITS,
      .stop_bits = UART_STOP_BITS_1,
      .parity = UART_PARITY_DISABLE,
      .tx_pin = 17, /* <-- Set secondary TX pin */
      .rx_pin = 18, /* <-- Set secondary RX pin */
  };
  s_uart_ext.rx_cb = NULL; /* Forwarding only, no RX callback needed */
  ESP_ERROR_CHECK(init_uart_driver(&s_uart_ext));

  ESP_LOGI(
      __func__,
      "External interface ready (CMD: GPIO%d, RDY: GPIO%d, Secondary UART1)",
      EXT_CMD_GPIO, EXT_RDY_GPIO);
}
...
static void print_active_coeff(filter_type_e t) {
  ...
  case _SOS_FLOAT: {
    ...
    for (int i = 0; i < f->Nsos; i++) {
      ...
      ESP_LOGI(__func__, "sosf%d: b=%f %f %f a=1 %f %f", i, s->b[0], s->b[1],
               s->b[2], s->a[1], s->a[2]);
    }
    break;
  }
  case _SOS_INT: {
    ...
    for (int i = 0; i < f->Nsos; i++) {
      ...
      ESP_LOGI(__func__, "sosi%d: Sb=%ld %ld %ld Sa=%ld %ld %ld", i,
               (long)s->Sb[0], (long)s->Sb[1], (long)s->Sb[2], (long)s->Sa[0],
               (long)s->Sa[1], (long)s->Sa[2]);
    }
    break;
  }
  case _LTC_FLOAT: {
...
static inline void fts_cfg_apply_pending(void) {
  ...
  /* Handle Execution Mode change (Internal <-> External) */
  if (g_fts_cfg.exe_mode_pending) {
    g_fts_cfg.exe_mode_pending = false;
    g_fts_cfg.active_exe_mode = g_fts_cfg.next_exe_mode;

    if (g_fts_cfg.active_exe_mode == FTS_EXE_MODE_EXT) {
      /* Stop internal timer in External Mode */
      gptimer_stop(gptimer);
      g_fts_cfg.timer_state = FTS_TIMER_STOPPED;
      ESP_LOGI(__func__, "Switched to EXTERNAL Mode: GPTimer Stopped");
    } else {
      /* Restart internal timer for Internal Mode */
      gptimer_start(gptimer);
      g_fts_cfg.timer_state = FTS_TIMER_RUNNING;
      ESP_LOGI(__func__, "Switched to INTERNAL Mode: GPTimer Started");
    }
  }
  ...
  if (g_fts_cfg.sim_freq_pending) {
    ...
    ESP_LOGI(__func__, "SIM freq -> %.1f Hz",
             (double)g_fts_cfg.active_sim_freq_hz);
  }
  ...
  if (g_fts_cfg.input_pending) {
    ...
    ESP_LOGI(__func__, "Input -> %s",
             g_fts_cfg.active_input == FTS_INPUT_ADC ? "ADC" : "SIM");
  }

  if (reinit_filter) {
    df_init(&s_filter, g_fts_cfg.active_filter,
            filter_get_coeff(g_fts_cfg.active_filter), DEFAULT_DIRECT_FORM,
            DEFAULT_QUANT_MODE);
    g_coeff_override = NULL;
    ESP_LOGI(__func__, "Filter reinit -> type %d",
...
static int fmode_str_to_enum(const char *s) {
  static const char *names[] = {"iiri", "iirf", "firi", "firf",
                                "sosi", "sosf", "ltci", "ltcf"};
  static const filter_type_e vals[] = {_IIR_INT,   _IIR_FLOAT, _FIR_INT,
                                       _FIR_FLOAT, _SOS_INT,   _SOS_FLOAT,
                                       _LTC_INT,   _LTC_FLOAT};
  for (int i = 0; i < 8; i++)
    if (!strcmp(s, names[i]))
      return vals[i];
...
static void do_reset(void) {
  ...
  ESP_LOGI(__func__, "RESET: filter=%d input=%d freq=%.1f amp=%d mode=%d",
           ...
           (double)DEFAULT_SIM_FREQ_HZ, (int)DEFAULT_SIM_AMP,
           (int)DEFAULT_SIM_MODE);
}

static void forward_to_secondary_uart(const char *line) {
  /* Wait for RDY pin to go LOW */
  while (gpio_get_level(EXT_RDY_GPIO) != 0) {
    vTaskDelay(pdMS_TO_TICKS(1));
  }

  /* Drive CMD pin LOW */
  gpio_set_level(EXT_CMD_GPIO, 0);

  /* Wait a few milliseconds before starting transfer */
  vTaskDelay(pdMS_TO_TICKS(5));

  /* Forward command line with newline over secondary UART */
  uart_send(&s_uart_ext, (const uint8_t *)line, strlen(line));
  uart_send(&s_uart_ext, (const uint8_t *)"\n", 1);

  /* Wait for RDY pin to return HIGH */
  while (gpio_get_level(EXT_RDY_GPIO) != 1) {
    vTaskDelay(pdMS_TO_TICKS(1));
  }

  /* Drive CMD pin back HIGH */
  gpio_set_level(EXT_CMD_GPIO, 1);
}
...
static void uart_cmd_process_line(char *line) {
  ...
  if (!strcmp(line, "exemode:int")) {
    g_fts_cfg.next_exe_mode = FTS_EXE_MODE_INT;
    g_fts_cfg.exe_mode_pending = true;
    ESP_LOGI(__func__, "Mode -> INTERNAL");
    return;
  } else if (!strcmp(line, "exemode:ext")) {
    g_fts_cfg.next_exe_mode = FTS_EXE_MODE_EXT;
    g_fts_cfg.exe_mode_pending = true;
    ESP_LOGI(__func__, "Mode -> EXTERNAL");
    return;
  }

  // If EXTERNAL mode, ALSO forward the command :
  if (g_fts_cfg.active_exe_mode == FTS_EXE_MODE_EXT) {
    forward_to_secondary_uart(line);
  }
  ...
  if (!strcmp(line, "reset")) {
    ...
    print_active_coeff(
        g_fts_cfg.active_filter); /* implement: dump struct via ESP_LOGI */
...
static void uart_cmd_rx_cb(uint8_t *data, uint16_t len) {
  ...
  while (1) {
    ...
    *newline = '\0';
    ...
    size_t remaining = rx_used - (line_len + 1);

    memmove(rx_accum, newline + 1, remaining);
...
_Static_assert(sizeof(dsm_packet_t) == DSM_PACKET_SIZE_BYTES,
               "DSM size invalid");
...
void app_main(void) {
  ...
  // UART command interface(uses your uart_driver component) * UART1, 115200 -
  // 8N1, GPIO 17 TX / 18 RX  – adjust pins as needed
  ...
  df_init(&s_filter, DEFAULT_FILTER_TYPE, filter_get_coeff(DEFAULT_FILTER_TYPE),
          DEFAULT_DIRECT_FORM, DEFAULT_QUANT_MODE);
...
#endif
    // INTERNAL MODE: Standard GPTimer-driven local DSP
    if (g_fts_cfg.active_exe_mode == FTS_EXE_MODE_INT) {
      ...
      filt_y = df_process(&s_filter, adc_raw);
      ...
      dt_us = esp_timer_get_time() - t1;
      data_ready = true; /* Mark sample ready for dispatch */
    } else {             // EXTERNAL MODE: RDY handshake + ADC_X / ADC_Y read
      /* 1. Wait for RDY pin to transition LOW */
      while (gpio_get_level(EXT_RDY_GPIO) != 0) {
        vTaskDelay(pdMS_TO_TICKS(1));
      }

      /* 2. Record low timestamp */
      int64_t t_rdy_low = esp_timer_get_time();

      /* 3. Read both analog input channels (ADC_X and ADC_Y) */
      int raw_x = 0, raw_y = 0;
      adc_oneshot_read(s_adc_handle, ADC_CHANNEL_4, &raw_x); /* ADC_X */
      adc_oneshot_read(s_adc_handle, ADC_CHANNEL_5, &raw_y); /* ADC_Y */

      adc_raw = (int16_t)(raw_x - 2048);
      filt_y = (int16_t)(raw_y - 2048);

      /* 4. Wait for RDY pin to transition back HIGH */
      while (gpio_get_level(EXT_RDY_GPIO) != 1) {
        vTaskDelay(pdMS_TO_TICKS(1));
      }

      /* 5. Record high timestamp and compute time difference */
      int64_t t_rdy_high = esp_timer_get_time();
      dt_us = t_rdy_high - t_rdy_low;
      ...
      s_dsm_pkt.fgen_out = adc_raw;
      s_dsm_pkt.filt_x = adc_raw;
      s_dsm_pkt.filt_y = filt_y; /* Assigned directly from ADC_Y */
      s_dsm_pkt.dac0 = (int16_t)(adc_raw + 2048);
      s_dsm_pkt.dac1 = (int16_t)(filt_y + 2048);
    }

    // UNIFIED DISPATCH PIPELINE (Runs once data_ready == true)
    if (data_ready) {
      /* Frame Packet Data */
      ...
      if (usb_cdc_is_connected(USB_CDC_PORT_0)) {
        usb_cdc_write(USB_CDC_PORT_0, (const uint8_t *)&s_dsm_pkt,
                      sizeof(s_dsm_pkt));
      }
#endif

#if (ACTIVE_SEND_MODE == SEND_MODE_UDP)
      memcpy(s_batch_buf + sizeof(dsm_batch_hdr_t) +
                 (s_batch_idx * sizeof(dsm_packet_t)),
             &s_dsm_pkt, sizeof(dsm_packet_t));
  ...
  if (cnt >= 100) {
    ESP_LOGI(
        __func__, "avg_period=%lld us  avg_exec=%lld us  filt=%d  input=%s",
        period_sum / (cnt - 1), exec_sum / cnt, (int)g_fts_cfg.active_filter,
        g_fts_cfg.active_input == FTS_INPUT_ADC ? "ADC" : "SIM");
