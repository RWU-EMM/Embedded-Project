    ...
    while (1)
    {
        for (int i = 0; i < 2; i++)
        {
            if (s_context[i].initialized)
            {
                ...
                esp_err_t err = tinyusb_cdcacm_write_flush(prv_map_port((usb_cdc_port_t)i), 0);

                if ((err != ESP_OK) &&
                    (err != ESP_ERR_TIMEOUT) &&
                    (err != ESP_ERR_NOT_FINISHED))
                {
                    ESP_LOGW("usb_cdc", "flush port %d: %s", i, esp_err_to_name(err));
                }
            }
        }
...
esp_err_t usb_cdc_driver_init(const usb_cdc_driver_config_t *config)
{
     ...
     * above the idle task so it yields readily.
     */
...
esp_err_t usb_cdc_write(usb_cdc_port_t port, const uint8_t *data, size_t length)
{
    ...
    {
        s_drop_count++;
        if ((s_drop_count % 1000) == 0)
            ESP_LOGW("usb_cdc", "port %d: %lu drops so far", port, (unsigned long)s_drop_count);
