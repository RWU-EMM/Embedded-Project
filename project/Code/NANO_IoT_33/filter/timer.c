...
typedef struct {
    uint16_t divisor;
    uint16_t ctrlb_prescaler; /* Fixed: Changed uint8_t to uint16_t to fit CTRLA prescaler bit shifts */
} timer_prescaler_t;
...
static bool timer_configure(uint32_t period_us)
{
    ...
    /* Enable APBC clock for TC3 */
    PM->APBCMASK.reg |= PM_APBCMASK_TC3;

    /* Configure GCLK0 (48MHz) for TC3 */
    ...
    /* Configure 16-bit Match Frequency (MFRQ) mode with prescaler */
